 ...
 */
public class RunGlobalScenarioOneSelecT{
	
	// settting this outside to have access from other classes
    	public static boolean DO_INTERMEDIATE	= false; // set this when you want to print out the intermediate

    public static void main(String[] args) {
        // by default this is en-es, but should be passed by cmd line
        String trans_map			= "translation_map_en-es.ser";
        String apertium_trans_map			= "translation_map_en-es.apertium.ser";
        String nematus_trans_map			= "translation_map_en-es.nematus.ser";
        String moses_trans_map			    = "translation_map_en-es.moses.ser";
        String predict_map			        = "fasttext.en";
        String predict_map_integers	        = "fasttext.en.pred";
        ...
        if(langpair==null){
            ...
            trans_map = "translation_map_files/translation_map_"+langpair+".ser";
            apertium_trans_map = "translation_map_files/"+apertium_trans_map;
            moses_trans_map = "translation_map_files/"+moses_trans_map;
            nematus_trans_map = "translation_map_files/"+nematus_trans_map;
            predict_map = "translation_map_files/"+predict_map;
            predict_map_integers = "translation_map_files/"+predict_map_integers;
            langCmd   = langpair;// default at en-es
            //langCmd   = "en-es";// default at en-es
            //trans_map = "translation_map_files/translation_map_es-fr-2.ser";
	    }
        if(testsource==null){
...
*/
	...
	Map<String,String> apertiumTranslations = new HashMap<String,String>();
	Map<String,String> mosesTranslations = new HashMap<String,String>();
	Map<String,String> nematusTranslations = new HashMap<String,String>();
	Map<String,String> predictions= new HashMap<String,String>();
	try{
        // load the translation map
        //System.err.println("loading default translation map" + trans_map);
        ...
        //System.err.println("loading apertium map" + apertium_trans_map);
		// load the apertium translation map
		fis = new FileInputStream(apertium_trans_map);
		ois = new ObjectInputStream(fis);
		apertiumTranslations = (Map<String,String>) ois.readObject();
		ois.close();
		fis.close();

		// load the moses translation map
        //System.err.println("loading moses map" + moses_trans_map);
		fis = new FileInputStream(moses_trans_map);
		ois = new ObjectInputStream(fis);
		mosesTranslations = (Map<String,String>) ois.readObject();
		ois.close();
		fis.close();
		
        // load the nematus translation map
        //System.err.println("loading nematus map" + nematus_trans_map);
		fis = new FileInputStream(nematus_trans_map);
		ois = new ObjectInputStream(fis);
		nematusTranslations = (Map<String,String>) ois.readObject();
		ois.close();
		fis.close();

        // load the predictions
        //System.err.println("loading predictions" + nematus_trans_map);
        BufferedReader enSent = new BufferedReader(new FileReader(predict_map));
        BufferedReader enPred = new BufferedReader(new FileReader(predict_map_integers));
        while (true) {
            String partOne = enSent.readLine();
            String partTwo = enPred.readLine();
            if (partOne == null || partTwo == null)
                break;
            predictions.put(partOne.toLowerCase(),partTwo.toLowerCase());
        }
    ...
    for(int i=0; i<stestsegs.size(); i++){
        ...
        //System.err.println("s prime:" + theSPrimeSegment.toString());
	    if(!DO_TRANS_MAP)
	    ...
	    if(stestsegs.get(i).size()<=SX_MAX_SENT_SIZE){
		    ...
		    if(bestScoreGotten && !DO_BASELINE){
				/* use this to print out s to s' aligments
				if(bestTalignment!=null){
					for(Pair<Integer,Integer> alignmentPair : bestTalignment)
					  System.err.print(alignmentPair.getFirst() + "-" + alignmentPair.getSecond() + " ");
					System.err.println();
					continue;
				}
				else
				...
				*/ 
				...
				List<PatchPhrasePair> phrasePairs = patchManager.getAllPhrasePairs(bestTUSourceSegment, theSPrimeSegment, 
									bestTalignment,SIGMA_MAX_MIN, SIGMA_MAX, SIGMA_MIN);
				...
				for(PatchPhrasePair phrase : phrasePairs){
					...
					if(!sPrime.toString().toLowerCase().equals(sX.toString().toLowerCase()) && sX.toString().length()>0){
                        ...
                        // 0 = moses 1 = apertium 2 = nematus
                        Integer predictedSystem = 0;
                        // going to use nematus for now 
                        if(predictions.containsKey(sX.toString().toLowerCase())){
                            predictedSystem = Integer.parseInt(predictions.get(sX.toString().toLowerCase()));
                            // check to make sure in moses
                            if(predictedSystem==0 && ! mosesTranslations.containsKey(sX.toString().toLowerCase())){
                                predictedSystem=2;
                            }
                            if(predictedSystem==0 && mosesTranslations.containsKey(sX.toString().toLowerCase())){
                                //System.err.println("USING MOSES");
                                translation = getTranslation(sX.toString(), langCmd, mosesTranslations,DO_TRANS_MAP, moses_trans_map);	
                            }
                            else if(predictedSystem==1){
                                //System.err.println("USING APERTIUM");
                                //translation = getTranslation(sX.toString(), langCmd, apertiumTranslations,DO_TRANS_MAP,apertium_trans_map);	
                                translation = getTranslation(sX.toString(), langCmd, nematusTranslations,DO_TRANS_MAP, nematus_trans_map);	
                            }
                            else if(predictedSystem==2){
                                //System.err.println("USING NEMATUS");
                                translation = getTranslation(sX.toString(), langCmd, nematusTranslations,DO_TRANS_MAP,nematus_trans_map);	
                            }
                            else{
                                //System.err.println("USING NOTHING");
						...
						if(translation!=null&&translation.length()>0){
							...
							if(tuContains!=null && tuContains.size()>0){
								for(Pair<Integer,Integer> matchedTu : tuContains){
									...
									Pair<Integer,Integer> origSPrimePositions = new Pair(fstart,fend); // s positions 
                                    // 0 = moses 1 = apertium 2 = nematus
                                    predictedSystem = 0;
									String finalSPrimeTranslation = null;	
                                    // going to use nematus for now 
                                    if(predictions.containsKey(sPrime.toString().toLowerCase())){
                                        predictedSystem = Integer.parseInt(predictions.get(sPrime.toString().toLowerCase()));
                                        // check to make sure in moses
                                        if(predictedSystem==0 && ! mosesTranslations.containsKey(sPrime.toString().toLowerCase())){
                                            predictedSystem=2;
                                        }
                                        if(predictedSystem==0 && mosesTranslations.containsKey(sPrime.toString().toLowerCase())){
                                            finalSPrimeTranslation = getTranslation(sPrime.toString(), langCmd, mosesTranslations,DO_TRANS_MAP, moses_trans_map);	
                                        }
                                        else if(predictedSystem==1){
                                            finalSPrimeTranslation = getTranslation(sPrime.toString(),langCmd, nematusTranslations,DO_TRANS_MAP, nematus_trans_map);	
                                            //finalSPrimeTranslation = getTranslation(sPrime.toString(), langCmd, apertiumTranslations,DO_TRANS_MAP,apertium_trans_map);	
                                        }
                                        else if(predictedSystem==2){
                                            finalSPrimeTranslation = getTranslation(sPrime.toString(), langCmd, nematusTranslations,DO_TRANS_MAP,nematus_trans_map);	
                                        }
                                            ...
                                            finalSPrimeTranslation = getTranslation(sPrime.toString(), langCmd, nematusTranslations,DO_TRANS_MAP,nematus_trans_map);	
                                            //finalSPrimeTranslation = getTranslation(sPrime.toString(), langCmd, translations,DO_TRANS_MAP,trans_map);	
                                        }
                                    }
                                    else{
                                            finalSPrimeTranslation = getTranslation(sPrime.toString(), langCmd, nematusTranslations,DO_TRANS_MAP,nematus_trans_map);	
									...
									if(!DO_TRANS_MAP){
										...
										if(USE_PATCH_RATIO){
											...
											if((tauPrimeSize<=USE_PATCH_RATIO_TAU_PRIME) && (tauSize <=USE_PATCH_RATIO_TAU)){ 
												...
												patchOpOne.setOrigSPrimePos(origSPrimePositions);
...
/***************
				// compare against mt
				int mtBestEd = Segment.EditDistance(theMTSegment, theGoldSegment, null);
				int mtBestMaxWords=java.lang.Math.max(theMTSegment.size(),theGoldSegment.size());
...
***************************************/
			...
			if(patchOps.size()>0){
				...
				if(patchOps.size()<MAX_PATCHOPS_SIZE){
					...
					//Patch.printOps(patchOps);
		    			String toPrint = 
						i + "\t" +
						theSPrimeSegment.toString() + "\t" +
						theGoldSegment.toString()+"\t"+
						bestTUSourceSegment.toString()+"\t"+
						bestTUTargetSegment.toString();
					//System.out.println(toPrint);
					//System.err.println("attemptingpatch");
					
					patchManager.toPrint = toPrint;
	    ...
	    double lineWER = (bestEd/(double)bestMaxWords);
	    if(!DO_INTERMEDIATE){
		//System.err.println("LINEWER:" + lineWER);
	    }
	}// for test segs
    if(!DO_INTERMEDIATE)
		System.err.println("Relative Error is" + (totalError/totalWords));
	for (Integer key: TAU_COUNT.keySet()) {
	    int[] values = TAU_COUNT.get(key);
	    //System.err.println("TAU - LENGTH: " + key.toString() +" - " +  values[0]+ " - " + values[1]);
	}
	for (Integer key: SIG_COUNT.keySet()) {
	    int[] values = SIG_COUNT.get(key);
	    //System.err.println("SIGMA - LENGTH: " + key.toString() +" - " +  values[0]+ " - " + values[1]);
	}
...
//						System.err.println("Op words: "+op.getWords().toString());
	...
	if(translations!=null && translations.containsKey(input)){
        ...
        try {
            /** uncomment this if using file only **/
            System.out.println(">>>using apertium translate>>>"+input);
        	System.out.println(">>>translate>>>"+input);
		    if(true)
			    return "";
            /** uncomment this if using file only **/
