 ...
 * Glass box feature 11, feature 17 in count
 * Measure  of  how  evenly  distributed  are  the  mismatched  target-language  sub-
 * segments in the patching operators used to build the patched translation
 * @author John Evan Ortega
 * @version 0.1
 */
public class OutputFeature17 extends OutputFeature{
	public OutputFeature17(){}
	@Override
		...
		for(PatchOperator patchOp : super.iFeature.operators){
			...
			if(patchOp.isYesNode()){
				...
				for(Pair<Integer,Integer> pair : talignment){
					...
					if(wasMatched && alignCounter == talignment.size()){
						runNum = true;
					}
					if(runNum){
						...
						// 04/03/2017 - new from mikel
						denominator = tSubSegment.size() - (2 * prevMatched) + 1; 
				...
				// multiply * tau size
				if(topRightExpr>0){
					finalNumerator = tSubSegment.size() * (double)topRightExpr;
				}
