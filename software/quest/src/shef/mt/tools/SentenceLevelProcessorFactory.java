...
public class SentenceLevelProcessorFactory {
    ...
    public SentenceLevelProcessorFactory(FeatureExtractor fe) {
        ...
        if (requirements.contains("fmr.path")) {
            //Get alignment processors:
            FMRProcessor fmrProcessor = this.getFMRProcessor();

            //Add them to processor vectors:
            sourceProcessors.add(fmrProcessor);
    ...
    private FMRProcessor getFMRProcessor() {
        ResourceManager.registerResource("fmr.path");
        String fmrPath = this.fe.getResourceManager().getString("fmr.path");
        FMRProcessor fmrProc = new FMRProcessor(fmrPath);
        return fmrProc;
    }
