...
# or call like ./run_experiments.sh en_es 80 esplagomis dev 100 where 100 is the number of trees for ETR
...
echo "Running the __${alter}__ training set with ${2} fms and using __${runset}__ for testing with __${numtrees}__trees"

## uncomment if you are loading files
## you will need to change john.cfg also to loadfile
#if [ "${alter}" = "esplagomis" ]; then
#    cp model_file_combined_new_nobb10/model_file_new_90_fms_Etr_test.nofs.100t.esplagomis_*.pkl model_file.pkl
    #cp model_file_${1}_new/model_file_new_${2}_*.esplagomis_*.pkl model_file.pkl
#    cp model_file_${1}_new/model_file_new_${2}_fms_Etr_test.nofs.100t.esplagomis_*.pkl model_file.pkl
    #cp model_file_${1}_new/model_file_new_60_fms_Etr_test.nofs.esplagomis_*.pkl model_file.pkl
#else
#    cp model_file_combined_new_nobb10/model_file_new_90_fms_Etr_test.nofs.100t.none_*.pkl model_file.pkl
    #cp model_file_${1}_new/model_file_new_${2}_fms_Etr_test.nofs.100t.none_*.pkl model_file.pkl
    #cp model_file_${1}_new/model_file_new_${2}_fms_Etr_test.nofs.100t.none_*.pkl model_file.pkl
    #cp model_file_${1}_new/model_file_new_${2}_*.none_*.pkl model_file.pkl
    #cp model_file_${1}_new/model_file_new_60_fms_Etr_test.nofs.none_*.pkl model_file.pkl
#fi
...
if [ "${alter}" = "esplagomis" ]; then
	gzip -f run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new.esplagomis
	gzip -f run_global_scenario_${1}_${2}.dev.bbandbothgrounding.wer.new.esplagomis
	gzip -f run_global_scenario_${1}_${2}.test.bbandbothgrounding.wer.new.esplagomis
	gzip -f run_global_scenario_combined_60.train.bbandbothgrounding.wer.new.esplagomis
    ## uncomment for a train on 60% fms every time
	./prepare_for_quest.sh run_global_scenario_combined_60.train.bbandbothgrounding.wer.new.esplagomis.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.esplagomis.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.sh run_global_scenario_${1}_60.train.bbandbothgrounding.wer.new.esplagomis.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.esplagomis.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.sh run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new.esplagomis.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.esplagomis.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.random.sh run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new.esplagomis.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.esplagomis.gz $numtrees > prepare.out 2>&1
	sleep 10
	awk -F'\t' {'print $36'} run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.esplagomis | sort | uniq > grep1.o 
else
	gzip -f run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new
	gzip -f run_global_scenario_${1}_${2}.dev.bbandbothgrounding.wer.new
	gzip -f run_global_scenario_${1}_${2}.test.bbandbothgrounding.wer.new
	gzip -f run_global_scenario_combined_60.train.bbandbothgrounding.wer.new
    ## uncomment for a train on 60% fms every time
	./prepare_for_quest.sh run_global_scenario_en_es_60.test.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.sh run_global_scenario_combined_60.train.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.sh run_global_scenario_${1}_60.train.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.sh run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	#./prepare_for_quest.random.sh run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	sleep 10
...
model_file_dir=model_file_combined_new_nobb10
#model_file_dir=model_file_${1}_new
model_file="model_file_new_${2}_fms_Etr_test.nofs.${numtrees}t.${alter}_${tmwer}_${fmrwer}_${orawer}_${successrate}.pkl"
