...
if [ "${alter}" = "esplagomis" ]; then
    cp model_file_${1}_new/model_file_new_60train_${2}_fms_Etr_test.nofs.esplagomis_*.pkl model_file.pkl
#    cp model_file_${1}/model_file_60fms_etr_nofs.esplagomis_*.pkl model_file.pkl
else
    cp model_file_${1}_new/model_file_new_60train_${2}_fms_Etr_test.nofs.none_*.pkl model_file.pkl
...
if [ "${alter}" = "esplagomis" ] && [ "${switchtest}" = "" ]; then
	...
	./prepare_for_quest.sh run_global_scenario_${1}_60.train.bbandbothgrounding.wer.new.esplagomis.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.esplagomis.gz > prepare.out 2>&1
	sleep 10
	...
	./prepare_for_quest.sh run_global_scenario_${1}_${2}.train.bbandbothgrounding.wer.new.esplagomis.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz > prepare.out 2>&1
	sleep 10
	awk -F'\t' {'print $36'} run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer | sort | uniq > grep1.o 
else
	...
	gzip -f run_global_scenario_${1}_${2}.test.bbandbothgrounding.wer.new
    ## uncomment for a train on 60% fms every time
	./prepare_for_quest.sh run_global_scenario_${1}_60.train.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.bbandbothgrounding.wer.new.gz > prepare.out 2>&1
...
model_file="model_file_new_60train_${2}_fms_Etr_test.nofs.${alter}_${tmwer}_${fmrwer}_${orawer}_${successrate}.pkl"
