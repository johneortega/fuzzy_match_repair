...
cp ${train_file}.without.successrates features.dev.tz
cp ${train_file}.successrates success_rates.train.tz
cp ${train_file}.successrates success_rates.dev.tz
...
dev_sentences=${train_file}.sentences
dev_sentences_count=${train_file}.sentences.count
dev_sentences_wer=${train_file}.sentences.wer
#awk -F'\t' '{print $33}' ${train_file} > ${dev_sentences}
awk -F'\t' '{print $8}' ${train_file} > ${dev_sentences}
python process.py $dev_sentences > ${dev_sentences_count}
#awk -F'\t' '{print $34}' ${train_file} > ${dev_sentences_wer}
awk -F'\t' '{print $9}' ${train_file} > ${dev_sentences_wer}
