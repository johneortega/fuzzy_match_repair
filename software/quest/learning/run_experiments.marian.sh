...
if [ "${alter}" = "esplagomis" ]; then
    gzip -f run_global_scenario_${1}_${2}.${runset}.esplagomis.marian.bbandbothgrounding.wer.new
	./prepare_for_quest.sh run_global_scenario_${1}_${2}.${runset}.esplagomis.marian.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.esplagomis.marian.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	sleep 10
	awk -F'\t' {'print $36'} run_global_scenario_${1}_${2}.${runset}.esplagomis.marian.bbandbothgrounding.wer.new | sort | uniq > grep1.o 
else
	gzip -f run_global_scenario_${1}_${2}.${runset}.marian.bbandbothgrounding.wer.new
    
    ## uncomment for a train on 60% fms every time
	./prepare_for_quest.sh run_global_scenario_${1}_${2}.${runset}.marian.bbandbothgrounding.wer.new.gz run_global_scenario_${1}_${2}.${runset}.marian.bbandbothgrounding.wer.new.gz $numtrees > prepare.out 2>&1
	sleep 10
	awk -F'\t' {'print $36'} run_global_scenario_${1}_${2}.${runset}.marian.bbandbothgrounding.wer.new | sort | uniq > grep1.o 
fi
